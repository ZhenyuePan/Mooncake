   ...
   protected:
    void SetUp() override {
        ...
        auto status = transport_.install(local_segment_name_, metadata_,
                                         topology_, conf_);
        if (!status.ok()) {
            GTEST_SKIP() << "nvlink unavailable: " << status.ToString();
        }
        installed_ = true;
    ...
    void AllocateBatch(size_t capacity = 1) {
        ...
        nvlink_batch_ = dynamic_cast<NVLinkSubBatch*>(active_batch_);
        ASSERT_NE(nvlink_batch_, nullptr);
    }
    ...
    std::string local_segment_name_ = "nvlink-test-segment";
    Transport::SubBatchRef active_batch_ = nullptr;
    NVLinkSubBatch* nvlink_batch_ = nullptr;
    bool installed_ = false;
    void* src_dev_ = nullptr;
    void* dst_dev_ = nullptr;
};

TEST_F(NVLinkTransportTest, HostFuncDrivesSinkNotification) {
    AllocateBatch();
    ...
    EXPECT_FALSE(nvlink_batch_->events.empty());
    EXPECT_EQ(nvlink_batch_->events.size(), 1u);
}

TEST_F(NVLinkTransportTest, NoSinkDoesNotEnqueueHostFunc) {
...
TEST_F(NVLinkTransportTest, ClosedSinkBeforeFreeBecomesNoop) {
    AllocateBatch();
    ...
    ASSERT_EQ(cudaStreamSynchronize(nvlink_batch_->async_stream.get()),
              cudaSuccess);
...
TEST_F(NVLinkTransportTest, MultipleSubmitsAccumulateEventsAndNotify) {
    ...
    EXPECT_EQ(nvlink_batch_->events.size(), static_cast<size_t>(kSubmits));
